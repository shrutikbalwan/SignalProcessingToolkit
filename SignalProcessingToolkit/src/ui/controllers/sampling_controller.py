...
from src.ui.views.sampling_view import SamplingView


class SamplingController(BaseController):
    def __init__(self, event_bus: EventBus) -> None:
        self._service = SamplingService()
        self._viewmodel = SamplingViewModel(self._service)
        self._view: SamplingView | None = None
        super().__init__(event_bus)

    def get_view(self) -> SamplingView:
        if self._view is None:
            self._view = SamplingView(self._viewmodel)
        return self._view
